...
static gint          ezdfile_detect        (const gchar *filename,
                                            gboolean only_name);
...
static gint
ezdfile_detect(const gchar *filename, gboolean only_name)
{
    gint score = 0;
    FILE *fh;
    ...
    if (only_name) {
        ...
        score = (g_str_has_suffix(filename_lc, EXTENSION1)
                 || g_str_has_suffix(filename_lc, EXTENSION2)) ? 20 : 0;
        g_free(filename_lc);
    ...
    if (ok) {
        if ((i = find_data_offsets(buffer + header_size, size - header_size,
            ...
            if ((i = select_which_data(ezdfile, i)) != (guint)-1) {
                ...
                dfield = GWY_DATA_FIELD(gwy_data_field_new(section->xres,
                                                           section->yres,
                                                           1.0, 1.0, FALSE));
    ...
    while ((line = gwy_str_next_line(&buffer))) {
        ...
        *p = '\0';
        ...
        if (!strcmp(line, "SaveMode")) {
            ...
            if (!strcmp(p, "Scan forward"))
                section->direction = SCAN_FORWARD;
            else if (!strcmp(p, "Scan backward"))
           ...
           GwyContainer *container)
{
    ...
    /* Fix value scale */
    siunit = GWY_SI_UNIT(gwy_si_unit_new_parse(section->zrange.unit, &power10));
    ...
    /* Fix lateral scale */
    siunit = GWY_SI_UNIT(gwy_si_unit_new_parse(section->xrange.unit, &power10));
    gwy_data_field_set_si_unit_xy(dfield, siunit);
    g_object_unref(siunit);
    gwy_data_field_set_xreal(dfield, pow10(power10)*section->xrange.range);

    siunit = GWY_SI_UNIT(gwy_si_unit_new_parse(section->yrange.unit, &power10));
                  ...
                  guint ndata)
{
    ...
    dfield = GWY_DATA_FIELD(gwy_data_field_new(section->xres, section->yres,
                                               1.0, 1.0, FALSE));
    read_data_field(dfield, section);
