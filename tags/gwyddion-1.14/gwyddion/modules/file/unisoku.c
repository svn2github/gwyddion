...
static gint          unisoku_detect         (const gchar *filename,
                                             gboolean only_name);
...
static gint
unisoku_detect(const gchar *filename,
               gboolean only_name)
{
    gint score = 0;
    FILE *fh;
    ...
    if (only_name) {
        ...
        score = g_str_has_suffix(filename_lc, EXTENSION_HEADER) ? 10 : 0;
        g_free(filename_lc);
