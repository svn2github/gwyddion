...
static gint             mifile_detect           (const gchar *filename,
                                                 gboolean only_name);
...
static gint
mifile_detect(const gchar *filename, gboolean only_name)
{
    gint score = 0;
    FILE *fh;
    ...
    if (only_name) {
        ...
        score = g_str_has_suffix(filename_lc, EXTENSION) ? 100 : 0;
        g_free(filename_lc);
...
mifile_load(const gchar *filename)
{
    ...
    if (ok) {
        dfield = GWY_DATA_FIELD(gwy_data_field_new(mifile->xres,
                                                   mifile->yres,
                                                   1.0, 1.0, FALSE));
    ...
    if (dfield) {
        ...
        process_metadata(mifile, i, GWY_CONTAINER(object));
    }
    mifile_free(mifile);
                 ...
                 GwyContainer *container)
{
    ...
    if (bufferUnit) {
        siunit = GWY_SI_UNIT(gwy_si_unit_new_parse(bufferUnit, &power10));
