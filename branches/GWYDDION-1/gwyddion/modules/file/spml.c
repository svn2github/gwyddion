...
static gint spml_detect(const gchar *filename, gboolean only_name);
...
query_for_datachannel_name(char *filename)
{
    ...
    dialog_channellist =
        gtk_dialog_new_with_buttons("Select datachannel to import", NULL,
                                    GTK_DIALOG_MODAL, GTK_STOCK_CANCEL,
                                    GTK_RESPONSE_CANCEL, GTK_STOCK_OK,
                                    GTK_RESPONSE_OK, NULL);
    gtk_widget_set_size_request(GTK_WIDGET(dialog_channellist), 550, 300);
    gtk_dialog_set_default_response(GTK_DIALOG(dialog_channellist),
                                    GTK_RESPONSE_OK);
    ...
    switch (response) {
        case GTK_RESPONSE_OK:
            if (gtk_tree_selection_get_selected
                (gtk_tree_view_get_selection(GTK_TREE_VIEW(list_datachannels)),
                 &model, &iter)) {
...
spml_load(const gchar *filename)
{
    ...
    gint x_power10, y_power10, z_power10;
    ...
    num_of_data_dimension =
        get_data(FALSE, (char *)filename, channel_name, &data, &dimensions,
                 &unit, &scattered);
    ...
    siunit = gwy_si_unit_new_parse(g_array_index(units, xmlChar *, 0),
                                   &x_power10);
    g_object_unref(siunit);

    siunit = gwy_si_unit_new_parse(g_array_index(units, xmlChar *, 1),
                                   &y_power10);
