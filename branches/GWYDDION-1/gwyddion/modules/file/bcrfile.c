...
bcrfile_load(const gchar *filename)
{
    ...
    dfield = file_load_real(buffer, size, meta, &voidmask);
    gwy_file_abandon_contents(buffer, size, NULL);
    if (dfield) {
               ...
               GwyDataField **voidmask)
{
    ...
    if ((s = g_hash_table_lookup(meta, "xlength"))
        ...
        siunit1 = GWY_SI_UNIT(gwy_si_unit_new_parse(s, &power10));
        q *= pow10(power10);
    ...
    if ((s = g_hash_table_lookup(meta, "ylength"))
        ...
        siunit2 = GWY_SI_UNIT(gwy_si_unit_new_parse(s, &power10));
        q *= pow10(power10);
                           ...
                           GwyDataField **voidmask)
{
    ...
    dfield = GWY_DATA_FIELD(gwy_data_field_new(xres, yres, 1e-6, 1e-6, FALSE));
    *voidmask = GWY_DATA_FIELD(gwy_data_field_new(xres, yres, 1e-6, 1e-6,
                                                  TRUE));
    data = gwy_data_field_get_data(dfield);
