...
static gint          witec_detect             (const gchar *filename,
                                               gboolean only_name);
...
static gint
witec_detect(const gchar *filename,
             gboolean only_name)
{
    gint score = 0;
    struct stat st;
     ...
     * WIT file. */
    magic = g_new(gchar, WITEC_SIZE_HEADER);
    if (fread(magic, WITEC_SIZE_HEADER, 1, fh)) {
        WITecHeader header;
        const guchar *p = magic;
        gsize len = WITEC_SIZE_HEADER;
        gsize expected;
                          ...
                          guint i)
{
    ...
    dfield = GWY_DATA_FIELD(gwy_data_field_new
                                    (xres, yres,
    ...
    siunit = GWY_SI_UNIT(gwy_si_unit_new_parse(unit, &power10));
