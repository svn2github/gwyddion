...
indent_analyze_dialog(GwyContainer *data, IndentAnalyzeArgs * args)
{
    ...
    gwy_table_attach_hscale(table, row, _("Marked _areas:"), NULL,
                            GTK_OBJECT(controls.w_what_mark),
                            GWY_HSCALE_WIDGET);
    row++;


    controls.w_indentor
        = gwy_option_menu_indentor(G_CALLBACK(indentor_changed_cb), &controls,
                                   args->indentor);
    gwy_table_attach_hscale(table, row, _("_Indentor type:"), NULL,
                            GTK_OBJECT(controls.w_indentor),
                            GWY_HSCALE_WIDGET);
    row++;
    ...
    gwy_table_attach_hscale(table, row, _("_Mask creation type:"), NULL,
                            GTK_OBJECT(controls.w_how_mark),
                            GWY_HSCALE_WIDGET);
    row++;

    controls.w_plane_tol = gtk_adjustment_new(args->plane_tol,
                                              0, 100, 0.1, 1, 10);
    spin = gwy_table_attach_hscale(table, row, _("Ref. plane _tolerance:"), "%",
                                   controls.w_plane_tol, 0);
    row++;

    controls.w_phi_tol = gtk_adjustment_new(args->phi_tol,
                                            0, 180, 0.1, 10, 10);
    spin = gwy_table_attach_hscale(table, row, _("Angle _1 tolerance:"), "deg",
                            ...
                            gdouble *by)
{
    ...
    gwy_data_field_area_fit_plane(dfield, 0, 0, dfield->xres - 1, size,
                                  &cc, bx, NULL);
    gwy_data_field_area_fit_plane(dfield, 0, 0, size, dfield->yres - 1,
                                  c, NULL, by);
    *c = (*c + cc)/2;
}
                      ...
                      GwyDataField *dfield, FloodFillInfo * ffi)
{
    ...
    while (head != tail) {
        ...
        data_field_average_normal_vector(dfield, tail->x, tail->y,
                                         ffi->seed, &v);
        test = 0;
        switch (ffi->mode) {
            case NORMAL:
        ...
        if (test) {
            ...
            for (rr = -1; rr <= 1; rr++) {
                for (s = -1; s <= 1; s++) {
                    if (s || rr) {
                        val =
                            gwy_data_field_get_val(indentmask, tail->x + rr,
                                                   tail->y + s);
                        if (!val) {
                            head->x = tail->x + rr;
                            head->y = tail->y + s;
...
indent_analyze_do_the_hard_work(IndentAnalyzeControls * controls)
{
    ...
    if (args->what_mark == GWY_WHAT_MARK_POINTS) {
        gwy_data_field_area_fill(mask, args->minx - 2, args->miny - 2,
                                 args->minx + 2, args->miny + 2, 1.0);
        gwy_data_field_area_fill(mask, args->maxx - 2, args->maxy - 2,
                                 args->maxx + 2, args->maxy + 2, 1.0);
    }
    ...
    for (i = 0; i < args->nof_sides; i++) {
        ...
        if (args->what_mark == GWY_WHAT_MARK_POINTS) {
            gwy_data_field_area_fill(mask, sx - 2 - i, sy - 2 - i, sx + 2 + i,
                                     sy + 2 + i, 1.0);
        }
        else {
            ffi.mode = NORMAL;
...
update_data_labels(IndentAnalyzeControls *c)
{
    ...
    GwyDataField *dfield =
        GWY_DATA_FIELD(gwy_container_get_object_by_name(c->mydata, "/0/data"));
    GwySIValueFormat *siformat =
        gwy_data_field_get_value_format_xy(dfield, NULL);
    gdouble mag = siformat->magnitude;

    compute_expected_indent(c);

    sprintf(str, "[%d, %d]px: %lf", c->args->minx, c->args->miny,
            c->args->min_val/mag);
    gtk_label_set_text(GTK_LABEL(c->w_min_xy), str);

    sprintf(str, "[%d, %d]px: %lf", c->args->maxx, c->args->maxy,
            c->args->max_val/mag);
    gtk_label_set_text(GTK_LABEL(c->w_max_xy), str);

    sprintf(str, "%lf", (c->args->max_val - c->args->min_val)/mag);
