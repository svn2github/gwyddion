 ...
 *
 * Numbers from continous probability distributions generated by #GwyRand are
 * always from the open support interval.  In other words, they are never
 * exactly equal to the interval boundaries even for distributions where the
 * probability density function is nonzero at the boundary such as uniform or
 * one-side exponential.  In addition, they are never exactly zero.  See
 * gwy_rand_double() for some discussion.  Generators of numbers from other
