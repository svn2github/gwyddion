...
/**
 * gwy_mask_field_mark_extrema:
 * @field: A two-dimensional mask field.
 * @heightfield: A two dimensional data field.
 ...
 **/
void
gwy_mask_field_mark_extrema(GwyMaskField *field,
                            const GwyField *heightfield,
                            gboolean maxima)
{
    g_return_if_fail(GWY_IS_MASK_FIELD(field));
    g_return_if_fail(GWY_IS_FIELD(heightfield));
    guint xres = field->xres, yres = field->yres;
    g_return_if_fail(heightfield->xres == xres);
    g_return_if_fail(heightfield->yres == yres);

    gwy_mask_field_fill(field, NULL, FALSE);

    gdouble min, max;
    gwy_field_min_max_full(heightfield, &min, &max);
    // This takes care of 1×1 fields too.
    if (min == max)
        return;

    guint *types = g_new0(guint, xres*yres);
    guint unmarked = (maxima ? mark_maxima : mark_minima)(heightfield, types);

    if (unmarked)
        propagate_non_extrema_marking(types, heightfield->data, xres, yres);
    ...
    for (guint k = 0; k < xres*yres; k++) {
        if (!(types[k] & 1))
            gwy_mask_field_set(field, k % xres, k/xres, TRUE);
    }
